...
def build_markdown(summary: dict) -> str:
    lines = [
        ...
        "",
        "| Package | Status | Exit | Timed Out | Crash Lines |",
        "|---|---|---:|---:|---:|",
    ]
    for row in summary.get("results", []):
        lines.append(
            f"| {row.get('package', 'unknown')} | {row.get('status', 'unknown')} | {row.get('monkey_exit_code', 'n/a')} | {str(row.get('timed_out', False)).lower()} | {row.get('crash_count', 0)} |"
        )
    return "\n".join(lines) + "\n"
