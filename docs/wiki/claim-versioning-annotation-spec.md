...
Security research framework for tracking claim supersessions, amendments, retractions, and confidence changes across analysis revisions.
...
| effective_revision | integer | YES | `1` | Revision cycle number when issued (1=initial, 2+=post-transfer-validation) |
...
| author_id | string | YES | `security_revision_1_ops` | Operator/revision cycle that issued revision |
...
|---|---|---|---|---|
| **initial** | First revision cycle, finding is published | — | RETAINED | Claim originates; initial discovery |
...
- `effective_revision` is monotonically non-decreasing across revision chain
- Revision cycle 1 = initial publication; cycle 2+ = post-transfer-validation updates only
...
- Sorted by effective_revision ascending, then revision_id ascending
- Enable readers to see evolution: "originally HIGH confidence, downgraded in revision cycle 2"
...
- Preface with revision-cycle label: "[Cycle 1] mock-only; [Cycle 2] ordering may be application-layer"
...
- Retracted claims (FALSIFIED) must be published in full in appendix with:
  ...
  - Example: "Originally ORDERING_BYPASS_001.r1 claimed protocol rejects pre-SESSION opcodes. Real-endpoint testing (endpoint_logs/20260920_run5.log lines 234–240) shows HELLO before SESSION is accepted and processed normally. Retraction issued revision cycle 2."
...
```json
{
  ...
  "author_id": "security_revision_1_ops"
}
```
...
```json
{
  ...
  "author_id": "security_revision_2_ops"
}
```
...
- [ ] Author_id identifies operator/revision cycle
...
**This specification enables auditable, conservative claim evolution across research revision cycles without loss of proof trail.**
